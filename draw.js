...
function draw() {
	...
	// 輪部分
	{
		...
		let d = 5;
		let divR = 100;
		let c = new Cylinder(divR, 96);
		c.addScale(0, d, d);
		c.addScale(1, d, d);
		let q = new qtn();
		q.setRot(new vec3(0,0,1), 0);
		c.addRot(0, q);
		q.setRot(new vec3(0,0,1), -Math.PI/2);
		c.addRot(0.25, q);
		q.setRot(new vec3(0,0,1), -Math.PI*2);
		c.addRot(1, q);
		for (let i=0; i<=divR; i++) {
			let th = 2*Math.PI*i/divR;
			let x = Math.cos(th)*r;
			let y = Math.sin(th)*r;
			c.addTranslate(i/divR, x, y, 0);
		}
		objList.push(c.createObject("輪"));
	}
