...
class vec2 {
	 ...
	 */
	normalize(scale=1) {
		let k = Math.sqrt(this.x*this.x + this.y*this.y);
		if (k) {
			k = scale / k;
		}
...
class mat4 {
	...
	setTranslate(ma, v) {
		let a = ma.#array;
		let d = this.#array;
		d[12] = a[0] * v.x + a[4] * v.y + a[8]  * v.z + a[12];
		d[13] = a[1] * v.x + a[5] * v.y + a[9]  * v.z + a[13];
		d[14] = a[2] * v.x + a[6] * v.y + a[10] * v.z + a[14];
		d[15] = a[3] * v.x + a[7] * v.y + a[11] * v.z + a[15];
	...
	setView(azimuth, tilte, eye, position) {
		let e = new vec3(eye[0], eye[1], eye[2]);
		let n = new vec3(Math.sin(tilte), Math.cos(tilte));
		let z = e.normalize();
...
class qtn {
	...
	setSlerp(qa, qb, time) {
		...
		if (idot <= 0.0) {
			...
			let pd = Math.acos(dot);
			let pt = pd * time;
			let k = 1.0 / Math.sqrt(idot);
			let t0 = k * Math.sin(pd - pt);
			let t1 = k * Math.sin(pt);
			d[0] = a[0]*t0 + b[0]*t1;
			d[1] = a[1]*t0 + b[1]*t1;
			d[2] = a[2]*t0 + b[2]*t1;
			d[3] = a[3]*t0 + b[3]*t1;
	 ...
	 * @param {vec3} v ベクトルv
	 */
	rotVec3(returnValue, v) {
		let x = this.#array[0],
			y = this.#array[1],
			z = this.#array[2],
			w = this.#array[3]
		;
		let tx = w*v.x - y*v.z + z*v.y,
			ty = w*v.y - z*v.x + x*v.z,
			tz = w*v.z - x*v.y + y*v.x,
			tw = x*v.x + y*v.y + z*v.z
		;
		returnValue.x = tx*w + tw*x + ty*z - tz*y;
		returnValue.y = ty*w + tw*y + tz*x - tx*z;
		returnValue.z = tz*w + tw*z + tx*y - ty*x;
	}
}
